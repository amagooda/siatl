...
def sum_clf(dataset, config, opts, transfer=False):
    from logger.experiment import Experiment

    opts.name = config["name"]
    X_train, y_train, posts_train, X_val, y_val, posts_val = dataset
    vocab = None
    if transfer:
    ...
    train_set = SUMDataset(X_train, posts_train, y_train,
                           vocab=vocab, preprocess=preprocessor,
                           vocab_size=config["vocab"]["size"],
                           seq_len=config["data"]["seq_len"], post_len=config["data"]["post_len"])

    print("Building validation dataset...")
    val_set = SUMDataset(X_val, posts_train, y_val,
                         seq_len=train_set.seq_len, preprocess=preprocessor,
                         vocab=train_set.vocab, post_len=train_set.post_len)

    src_lengths = [len(x) for x in train_set.data]
    val_lengths = [len(x) for x in val_set.data]

    train_post_lengths = [len(x) for x in train_set.data2]
    val_post_lengths = [len(x) for x in val_set.data2]
    ...
    model = SummarizationClassifier(ntokens, len(set(train_set.labels)), **config["model"])
    ...
    if config["model"]["has_att"] and config["model"]["has_coatt"]:
        top_parameters = filter(lambda p: p.requires_grad,
                                chain(model.top_rnn.parameters(),
                                      model.attention.parameters(),
                                      model.coattention.parameters(),
                                      model.classes.parameters()))

    elif config["model"]["has_att"]:
        top_parameters = filter(lambda p: p.requires_grad,
                                chain(model.top_rnn.parameters(),
                                      model.attention.parameters(),
                                      model.classes.parameters()))
    elif config["model"]["has_coatt"]:
        top_parameters = filter(lambda p: p.requires_grad,
                                chain(model.top_rnn.parameters(),
                                      model.coattention.parameters(),
                                      model.classes.parameters()))
    ...
    trainer = SumClfTrainer(model, train_loader, val_loader,
